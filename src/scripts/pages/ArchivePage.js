...
function ArchivePage() {
  ...
  React.useEffect(() => {
    NotesApiSource.getArchivedNotes().then(({ data }) => {
      setNotes(data);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <section className="note-page">
      <NoteAddButton />
      <h2 className="note-page__title">{locale === 'id' ? 'Catatan Diarsipkan' : 'Archived Notes'}</h2>
      <NotesSearch keyword={keyword} keywordChange={onKeywordChangeHandler} />
      {loading ? <Loader /> : filteredNotes.length ?
        <NoteList notes={filteredNotes} /> : <p>{locale === 'id' ? 'Tidak ada catatan diarsipkan' : 'No archived notes'}</p>}
    </section>
  );
}
